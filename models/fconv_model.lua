...
FConvModel.generationAttention = argcheck{
    ...
    call = function(self, config, bsz)
        ...
        return function(state)
            ...
            --print("attnscores")
            --plp.dump(attnscores)
            local t = sum:div(#attnscores)
            --print("sum:div(#attnscores)",t)
            return t
        end
    end
}
...
FConvModel.generationDecode = argcheck{
    ...
    call = function(self, config, bsz,typee)
        ...
        return function(state, targetIn)
            ...
            if false then
                --print("cout:dim:",cout:dim(),"\ncout:size:",cout:size(1)
                --    ,cout:size(2),cout:size(3),
                --    "\n softmaxIn:dim:",softmaxIn:dim(),"\nsoftmaxIn:size :",softmaxIn:size(1)
                --    ,softmaxIn:size(2),softmaxIn:size(3)
                --    )
                --print(type(softmaxIn:clone()))
                --tt = torch.Tensor(softmaxIn:copy()):float()
                --print(tt)
                
                --print(tt)
            end
            conv =softmaxIn:float()
            --plp.dump(softmaxIn[1])
            --print(state.encoderOut[1])
            local results = (softmax:forward(outmodule:forward(softmaxIn)))
            --conv2 = table.pack(conv)
            --table.insert(results,conv2)
            return results,conv -- table.unpack(results) 
            -- return softmax:forward(outmodule:forward(softmaxIn))
        end
    end
}

FConvModel.generationDecode2 = argcheck{
    {name='self', type='FConvModel'},
    {name='config', type='table'},
    {name='bsz', type='number'},
    call = function(self, config, bsz)
        ...
        return function(state, targetIn)
            ...
            --plp.dump(softmaxIn[1])
            --print(state.encoderOut[1])
            return tt --softmax:forward(outmodule:forward(softmaxIn))
        end
    end
}
